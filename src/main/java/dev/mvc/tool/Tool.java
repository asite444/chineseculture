...
public class Tool {
  ...
  public static synchronized String unit(long size){
    ...
    if (size < 1024){ // 1 KB 이하,1024 byte 미만이면
      str = size + " Byte";
    }else if (size < 1024 * 1024){ // 1 MB 이하// 1048576  byte미만이면  KB
      str = (int)(Math.ceil(size/1024.0)) + " KB"; 
    }else if (size < 1024 * 1024 * 1024){ // 1 GB 이하,1,073,741,824 byte 미만이면 MB
  ...
  /**
   * HTML 특수 문자의 변경(진짜 테그가 아닌 단순 문자 테그로만 출력 되도록 하는것)
