...
@Component("dev.mvc.contents.ContentsProc")
  public class ContentsProc implements ContentsProcInter {
    ...
    public int create(ContentsVO contentsVO) {
      ...
      //System.out.println("create proc used");
      return cnt;
    }

    @Override
    public List<ContentsVO> list_by_chno(int chno) {
      List<ContentsVO> list = this.contentsDAO.list_by_chno(chno);
      return list;
    }
    @Override
    public List<ContentsVO> list_by_chno_search(HashMap<String, Object> hashMap) {
      List<ContentsVO> list = contentsDAO.list_by_chno_search(hashMap);
      
      for (ContentsVO contentsVO : list) { // 내용이 50자 이상이면 50자만 선택
...
//      str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
      ...
      if (now_grp >= 2){ // 현재 그룹번호가 2이상이면 이전 그룹으로 갈수 있는 링크 생성 
        str.append("<span class='span_box_1'><A href='"+list_file+"?&word="+word+"&now_page="+_now_page+"&chno="+chno+"'>이전</A></span>"); 
      } 
      ...
      for(int i=start_page; i<=end_page; i++){ 
        ...
        if (now_page == i){ // 목록에 출력하는 페이지가 현재페이지와 같다면 CSS 강조(차별을 둠)
          ...
          str.append("<span class='span_box_1'><A href='"+list_file+"?word="+word+"&now_page="+i+"&chno="+chno+"'>"+i+"</A></span>");   
        } 
      } 
      ...
      if (now_grp < total_grp){ 
        str.append("<span class='span_box_1'><A href='"+list_file+"?&word="+word+"&now_page="+_now_page+"&chno="+chno+"'>다음</A></span>"); 
      } 
      str.append("</DIV>"); 
    ...
    public List<ContentsVO> list_by_chno_search_paging(HashMap<String, Object> map) {
      ...
      List<ContentsVO> list = this.contentsDAO.list_by_chno_search_paging(map);
    ...
    public List<ContentsVO> list_by_chno_grid_search_paging(HashMap<String, Object> map) {
      ...
      List<ContentsVO> list = this.contentsDAO.list_by_chno_grid_search_paging(map);
    ...
    public int update_text(ContentsVO contentsVO) {
      ...
      System.out.println("update_text proc used");
      return cnt;
    }
    ...
    @Override
    public int delete_contents_by_all_chno(Map<String, Object> chnos_list) {
      int cnt=this.contentsDAO.delete_contents_by_all_chno(chnos_list);
      return cnt;
    }

    @Override
    public int count_by_chno(int chno) {
       int cnt=this.contentsDAO.count_by_chno(chno);
      return cnt;
    }
}