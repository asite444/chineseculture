...
<body>
  ...
  <DIV class='title_line'>
    <A href="../chinesegrp/list.do" class='title_link'>카테고리 그룹</A> > <A
      href="../chinese/list_by_chgrpno.do?chgrpno=${chinesegrpVO.chgrpno }"
      class='title_link'>${chinesegrpVO.chfield }</A> > <A
      href="./list_by_chno.do?chno=${chVO.chno }" class='title_link'>${chVO.chtitle }</A>
  </DIV>
  ...
  <DIV class='content_body'>
    ...
    <DIV style="text-align: right; clear: both;">
      <form name='frm' id='frm' method='get'
        action='./list_by_chno_grid_search_paging.do'>
        ...
        <c:if test="${param.word.length() > 0 }">
          <button type='button'
            onclick="location.href='./list_by_chno_grid_search_paging.do?chno=${chVO.chno}&word='">검색
            취소</button>
        </c:if>
      </form>
    </DIV>
          ...
          <c:when test="${size1 > 0}"> <!-- 파일이 존재하면 -->
            <c:choose> 
              <c:when test="${thumb1.endsWith('jpg') || thumb1.endsWith('png') || thumb1.endsWith('gif')}"> <!-- 이미지 인경우 -->
                ...
                ${title} ${cnt}     <br>
               <%--  <del><fmt:formatNumber value="${tklevel }"  /></del> --%>
               <span style="color:#737373; font-size: 1.0em;">${tklevel } </span>
                <span style="font-size: 1.0em;">${lecturetime } </span>
              </c:when>
