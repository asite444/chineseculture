...
<script type="text/javascript">
  ...
  function checkNickname() {
    ...
    var frm = $('#frm'); // id가 frm인 태그 검색
    var nickname = $('#nickname', frm).val(); // frm 폼에서 checknickname가 'id'인 태그 검색
    var params = '';
    var msg = '';

    if ($.trim(nickname).length == 0) { // id를 입력받지 않은 경우
      msg = '· 별명을 입력하세요.<br>· 별명 입력은 필수 입니다.<br>· 별명은 3자이상 권장합니다.';
      ...
      $.ajax({
        ...
        success: function(rdata) { // 서버로부터 성공적으로 응답이 온경우
          ...
          if (rdata.cnt > 0) {
            $('#modal_content').attr('class', 'alert alert-danger'); // Bootstrap CSS 변경
            msg = "이미 사용중인 별명 입니다.";
            $('#btn_close').attr("data-focus", "nickname");  // id 입력으로 focus 이동
          } else {
            $('#modal_content').attr('class', 'alert alert-success'); // Bootstrap CSS 변경
            msg = "사용 가능한 별명 입니다.";
            $('#btn_close').attr("data-focus", "tel");  // tel 입력으로 focus 이동
            // $.cookie('checkId', 'TRUE'); // Cookie 기록
          }
          
          $('#modal_title').html('별명 중복 확인'); // 제목 
          $('#modal_content').html(msg);        // 내용
          $('#modal_panel').modal();              // 다이얼로그 출력
        },
        // Ajax 통신 에러, 응답 코드가 200이 아닌경우, dataType이 다른경우 
...
<jsp:include page="../menu/top.jsp" flush='false' />
  ...
  <FORM name='frm' id='frm' method='POST' action='./create.do' class="form-horizontal">
    ...
    <div class="form-group">
      ...
      <div class="form-group">
      <label for="mname" class="col-md-2 control-label" style='font-size: 0.9em;'> 별명*</label>    
      <div class="col-md-10">
        <input type='text' class="form-control" name='nickname' id='nickname' 
                   value='' required="required" style='width: 30%;' placeholder="별명">
    ...
    <div class="form-group">
      ...
      <div class="col-md-10">
        ...
        <div class="form-group">
      <label for="zipcode" class="col-md-2 control-label" style='font-size: 0.9em;'>이메일</label>    
      <div class="col-md-10">
        <input type='text' class="form-control" name='email' id='email' 
                   value='' style='width: 30%;' placeholder="이메일">
      </div>
    </div>  
